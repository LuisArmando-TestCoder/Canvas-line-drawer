...
const setFullScreen = function setFullScreen({ element, trigger, callback }) {
  ...
  const ifs = () => document.isFullScreen || document.webkitIsFullScreen || document.mozIsFullScreen;
...
const getDistanceBetween = function getDistanceBetween({x: x1, y: y1}) {
  return {
    and({x: x2, y: y2}) {
      const [leg1, leg2] = [
        x1 - x2,
...
Object.keys(HTMLElement.prototype).filter(txt => new RegExp('^on').test(txt)).forEach(name => {
  const newName = name.replace(/^on/, 'when');
  HTMLElement.prototype[newName] = function (callback) {
    this.addEventListener(newName, callback);
  }
});